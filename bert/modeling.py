...
class BertForDependencyParsing(BertPreTrainedModel):
    def __init__(self, config, arc_space, type_space, num_labels):
        super(BertForDependencyParsing, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout_out = nn.Dropout2d(p=config.hidden_dropout_prob)
        self.num_labels = num_labels

        self.arc_h = nn.Linear(config.hidden_size, arc_space)
        self.arc_c = nn.Linear(config.hidden_size, arc_space)
        self.attention = BiAAttention(arc_space, arc_space, 1, biaffine=True)

        self.type_h = nn.Linear(config.hidden_size, type_space)
        self.type_c = nn.Linear(config.hidden_size, type_space)
        self.bilinear = BiLinear(type_space, type_space, self.num_labels)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, heads = None, labels = None, confidence = None, greedy_inference=False):
        # flat_input_ids = input_ids.view(-1, input_ids.size(-1))
        # flat_token_type_ids = token_type_ids.view(-1, token_type_ids.size(-1)) if token_type_ids is not None else None
        # flat_attention_mask = attention_mask.view(-1, attention_mask.size(-1)) if attention_mask is not None else None

        encoding_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)

        # apply dropout for output
        # [batch, length, hidden_size] --> [batch, hidden_size, length] --> [batch, length, hidden_size]
        encoding_output = self.dropout_out(encoding_output.transpose(1, 2)).transpose(1, 2)

        # output size [batch, length, arc_space]
        arc_h = F.elu(self.arc_h(encoding_output))
        arc_c = F.elu(self.arc_c(encoding_output))

        # output size [batch, length, type_space]
        type_h = F.elu(self.type_h(encoding_output))
        type_c = F.elu(self.type_c(encoding_output))
        ...
        arcs = self.dropout_out(arcs.transpose(1, 2)).transpose(1, 2)
        ...
        if heads is not None and labels is not None:
            ...
            if attention_mask is not None:
                ...
                loss_type = loss_type * attention_mask.unsqueeze(2)

                # if we have the label confidence, so we can use the confidence to control the loss
                if confidence is not None:
                    loss_arc = loss_arc * confidence.unsqueeze(2) * confidence.unsqueeze(1)
                    loss_type = loss_type * confidence.unsqueeze(2)
            ...
            loss_type = loss_type[batch_index, child_index, labels.t()][1:]
    ...
    def decode_MST(self, energies, lengths, leading_symbolic=0, labeled=True):
        """
    ...
    def is_uni_punctuation(self, word):
        match = re.match("^[^\w\s]+$]", word, flags=re.UNICODE)
        return match is not None


    def is_punctuation(self, word, pos, punct_set=None):
        if punct_set is None:
            return self.is_uni_punctuation(word)
        else:
            return pos in punct_set

    def score(self, words, postags, heads_pred, types_pred, heads, types,
            punct_set=None):
        ...
        for i in range(batch_size):
            ...
            for j in range(len(words[i])):
                ...
                if not self.is_punctuation(word, pos, punct_set):
                    total_nopunc += 1
                    if heads[i][j] == heads_pred[i][j]:
                        ucorr_nopunc += 1
                        if types[i][j] == types_pred[i][j]:
